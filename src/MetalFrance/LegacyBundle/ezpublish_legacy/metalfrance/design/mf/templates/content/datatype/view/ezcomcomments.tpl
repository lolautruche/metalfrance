...
{if $attribute.content.show_comments}
    ...
    {if $attribute.content.enable_comment}
        {def $can_add = fetch( 'comment', 'has_access_to_function', hash( 'function', 'add',
             ...
             $anonymous_user_id=ezini('UserSettings', 'AnonymousUserID' )
             $is_anonymous=$user.contentobject_id|eq( $anonymous_user_id )}
        {if $can_add}
        ...
        {elseif $is_anonymous}
            <div class="message-error">
                    <p>
                        {'Vous devez être connecté pour ajouter des commentaires'|i18n( 'extension/metalfrance' )}
                    </p>
            </div>
        {else}
            <div class="message-error">
