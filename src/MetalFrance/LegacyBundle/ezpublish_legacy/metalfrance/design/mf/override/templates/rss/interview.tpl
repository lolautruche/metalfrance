    <item>
      <title>{'Interview'|i18n( 'extension/metalfrance/rss' )} {$node.name|wash}</title>
      <link>{str_replace( '/layout/set/rss', '', $node.url_alias|ezurl( 'no', 'full' ) )}</link>
      <description>
        {def $description = $node.data_map.introduction.content.output.output_text|striptags|trim|simplify|shortenw( 400 )}
